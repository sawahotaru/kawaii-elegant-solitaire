...
対象は **可解判定ソルバー `src/utils/solver.ts`** です。ここが静かに壊れると、型エラーにもビルドエラーにもならないまま次の2つが起きます:

- 「解ける」と称して**実は詰んでいる盤面**を初心者・中級モードで配ってしまう（プレイヤーには自分が下手なのか盤面が悪いのか分かりません）
- 一括あがりが**非合法な手順**で盤面を進め、壊れた状態のまま勝ちになる

そこでテストは勝敗フラグを信じず、**`solveBoards` が返した手順を1手ずつ検証**します。判定はソルバーとは独立に書いてあり（空列に置けるのは K だけ／表向きは色違いの降順／どの時点でも52枚が過不足なく存在する等）、ソルバーの勘違いをソルバーの理屈で見逃さないようにしています。CI はテストが通ったときだけデプロイします。
